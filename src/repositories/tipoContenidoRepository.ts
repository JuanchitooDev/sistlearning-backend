...
class TipoContenidoRepository {
    async getAll(): Promise<TipoContenidoResponse> {
        try {
    ...
    async getAllByEstado(estado: boolean): Promise<TipoContenidoResponse> {
        try {
            const tipos = await TipoContenido.findAll({
                where: {
                    activo: estado
    ...
    async getById(id: number): Promise<TipoContenidoResponse> {
        try {
    ...
    async create(data: ITipoContenido): Promise<TipoContenidoResponse> {
        try {
    ...
    async update(id: number, data: ITipoContenido): Promise<TipoContenidoResponse> {
        try {
            const tipo = await TipoContenido.findByPk(id)
    ...
    async delete(id: number): Promise<TipoContenidoResponse> {
        try {
            const tipo = await TipoContenido.findByPk(id);
            if (!tipo) {
                return { result: false, message: 'Tipo de contenido no encontrado' };
...
export default new TipoContenidoRepository()